...
export default function ProfileModal({ open = false, onClose = () => {}, user = null, onLogout = () => {}, onUpdateUser = () => {} }) {
  ...
  useEffect(() => { if (user) saveProfileToLocal(user); }, [user]);
            ...
            {localProfile ? (
              <>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="modal-btn" onClick={() => { onUpdateUser(localProfile); setOpen(false); }} > Use saved profile </button>
                  <button className="modal-btn" onClick={() => { clearLocalProfile(); setOpen(false); window.dispatchEvent(new CustomEvent("leaderbox:profile-changed", { detail: { user: null } })); }} > Clear saved profile </button>
                </div>
              </>
            ) : null}
            <div style={{ display: "flex", gap: 8 }}>
      ...
      <div ref={panelRef} className={`profile-modal ${isOpen ? "open" : ""}`} aria-hidden={!isOpen}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ width: 96, height: 96, overflow: "hidden", borderRadius: 999, cursor: "pointer" }} onClick={() => {
              ...
              {(user.avatar || user.pfp) ? (
                <img src={user.avatar || user.pfp} alt="pfp" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", borderRadius: "50%" }} />
              ) : (
          ...
          <button className="modal-btn" onClick={() => (window.location.href = "/pages/EditStack")}>Pick your movie stack</button>
          ...
          <div style={{ padding: "8px", borderRadius: 8, background: "rgba(0,0,0,0.15)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div className="small">{user.draws || 0}</div>
            </div>
          </div>

          <button className="modal-btn" onClick={doDeleteProfile} disabled={busyDelete} style={{ background: "#b71c1c" }}>
          ...
          <div>
            ...
            <div className="level-bar" style={{ marginTop: 8 }}>
              {LEVELS.map(l => (
                <div key={l.level} className="level-pill" style={{ background: (l.level === level.level) ? "var(--accent)" : "transparent", color: (l.level === level.level) ? "var(--black)" : "var(--white)" }}>
                  {l.level}
                </div>
              ))}
            </div>
        ...
        <button className="modal-btn" onClick={() => handleCopyProfileLink(user)} style={{ marginTop: 8 }}>Copy profile link</button>
        <button className="modal-btn" onClick={doLogout}>Logout</button>

        {copied && (
          <div style={{ position: "absolute", left: "50%", transform: "translateX(-50%)", bottom: 18, background: "rgba(0,0,0,0.8)", padding: "8px 12px", borderRadius: 8, zIndex: 500, color: "var(--white)", fontWeight: 700 }}>
